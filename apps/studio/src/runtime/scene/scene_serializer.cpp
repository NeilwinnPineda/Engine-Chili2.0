...
namespace studio_runtime
{
    namespace
    {
        ...
        bool ReadSceneRenderSettings(JsonCursor& cursor, SceneRenderSettings& settings)
        {
            ...
            while (cursor.Peek() != '}')
            {
                ...
                else if (key == "spaceColor")
                {
                    if (!ReadColor(cursor, settings.spaceColor))
                    {
                        return false;
                    }
                }
    ...
    std::string SceneSerializer::SaveToText(const RuntimeWorld& world)
    {
        ...
        out << "    },\n";
        out << "    \"spaceColor\": ["
            << sceneRenderSettings.spaceColor.r << ", "
            << sceneRenderSettings.spaceColor.g << ", "
            << sceneRenderSettings.spaceColor.b << "]\n";
        out << "  },\n";
        out << "  \"entities\": [\n";
