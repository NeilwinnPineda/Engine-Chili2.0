...
#include "runtime/scene/studio_default_scene_template.hpp"
...
namespace studio_runtime
{
    ...
    void StudioRuntimeHost::ConfigureInputContexts()
    {
        ...
        studio.BindAction("Select", BindMouseButton(InputMouseButton::Left, false, false, false, true, false));
        studio.BindAction("MultiSelect", BindMouseButton(InputMouseButton::Left, false, true, false, true, false));
        studio.BindAction("OrbitCamera", BindMouseButton(InputMouseButton::Left, false, false, true, true, false));
        studio.BindAction("PanCamera", BindMouseButton(InputMouseButton::Middle, false, false, false, true, false));
    ...
    bool StudioRuntimeHost::Play(const ProjectRuntimeDesc& project, std::string& outError)
    {
        ...
        if (project.exportedArtifactPath.empty() && project.previewRuntimeName.empty())
        {
        ...
        {
            if (!m_runtimeLibrary.Load(project.exportedArtifactPath, outError))
            {
                DestroyAllScriptInstances();
                m_activeProject = ProjectRuntimeDesc{};
                return false;
            }
            m_activeRuntime = m_runtimeLibrary.GetRuntime();
        }
        else
        {
            // Studio-owned preview fallbacks remain available for editor-only
            // projects; project gameplay must arrive through the DLL path.
            ...
            m_activeRuntime = m_runtime.get();
        }
        m_activeRuntime->BeginPlay(project);
        ...
        const ViewportRect& viewportRect)
    {
        ...
        if (m_state == StudioRuntimePlayState::Paused)
        {
            // Preserve the project runtime's last submitted frame while paused.
            // Rebuilding the editor world here would make Studio preview diverge
            // from the loaded artifact precisely when execution is paused.
            return;
        }

        bool runtimeProvidedFrame = false;
        FramePrototype runtimeRenderFrame;
        if (m_activeRuntime)
        {
            RuntimeFrame frame;
            m_activeRuntime->Tick(deltaSeconds, input, frame);
            m_viewportText = frame.textOutput.empty() ? "Play Mode." : frame.textOutput;

            if (frame.hasRenderFrame)
            {
                runtimeRenderFrame = std::move(frame.renderFrame);
                runtimeProvidedFrame = true;
            }
        ...
        m_renderFrame = runtimeProvidedFrame ? std::move(runtimeRenderFrame) : BuildWorldFrame();
        m_hasRenderFrame = true;
    ...
    bool StudioRuntimeHost::RegisterScriptPrototype(const engine::script::ScriptPrototype& prototype)
    {
        return m_scriptModule.Registry().Register(prototype);
    }
    ...
    void StudioRuntimeHost::SetScriptEventSink(std::function<void(const engine::script::ScriptEvent&)> sink)
    {
        m_scriptEventSink = std::move(sink);
    }
    ...
    engine::script::ScriptContext StudioRuntimeHost::BuildScriptContext(EntityId entityId)
    {
        ...
        engine::script::ScriptContext context(
            ...
            {
                if (m_scriptEventSink)
                {
                    m_scriptEventSink(event);
                }
            });
